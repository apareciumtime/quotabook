...
<div class="bookshelf_create_playground">
    ...
    <form method="POST" action="{{ route('book_shelf_update_post', $bookshelf->id) }}" enctype="multipart/form-data">
        <div class="bookshelf_form_frame">
            @csrf
            <div class="input_unit">
                <h2>Book Shelf Name</h2>
                <div class="input_input">
                    <input required name="bookshelf_name" type="text" value="{{ $bookshelf->name }}">
                </div>
            </div>
                ...
                @else
                    @foreach ($f1books as $f1book)
                        <div class="book_added" draggable="true">
                            ...
                            <input required name="f1books[]" type="text" value="{{ $f1book->title }}" readonly>
                            <button type="button" class="remove_book" data-book-count="{{ $f1book->order }}" data-floor="1">remove</button>
                        </div>
                    @endforeach
                @endif
            </div>
                ...
                @else
                    @foreach ($f2books as $f2book)
                        <div class="book_added" draggable="true">
                            ...
                            <input required name="f2books[]" type="text" value="{{ $f2book->title }}" readonly>
                            <button type="button" class="remove_book" data-book-count="{{ $f2book->order }}" data-floor="2">remove</button>
                        </div>
                    @endforeach
                @endif
            </div>
                ...
                @else
                    @foreach ($f3books as $f3book)
                        <div class="book_added" draggable="true">
                            ...
                            <input required name="f3books[]" type="text" value="{{ $f3book->title }}" readonly>
                            <button type="button" class="remove_book" data-book-count="{{ $f3book->order }}" data-floor="3">remove</button>
                        </div>
                    @endforeach
                @endif
            </div>
...
<script>
    $(document).ready(function() {
        var f1books = @json($f1books);
        var f2books = @json($f2books);
        var f3books = @json($f3books);

        var bookCounts = [f1books.length, f2books.length, f3books.length];
